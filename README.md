...
### Design
I've always wanted to explore the implementation of interactive terrain in games. Especially after being impressed by the [interactive snow scene in rise of the tomb raider](https://www.youtube.com/watch?v=QSYkwdlDN8s), I was so happy to have this opportunity in this course to explore the implementation of deformable snow. In order to implement this technical attempt, we designed the core gameplay of tower defense in a snow scene in the combination with the art's expectation of  art style of our game.
To combine snow deformation with the core gameplay, we designed a game mechanism that can collect snow from the the snowy ground and use it to slow down enemies. This not only demonstrates our technical goals, but is also a mechanism to make the game unique and fun.
...
1. We chose a borderland-like cartoon style as our art style. I was responsible for the development of cel shading post process and outline post process to achieve this graphical effect.
...
4. I developed landscape material so that designers and our artists can use the built-in landscape tools to easily customize the landscape. The mechanism of snow deformation can be automatically applied on where the terrain is mapped with snow. This gives us a lot of freedom in the design of the terrain and the scene.
...
## Technical goals
### Individual goals
1. The implementation of snow deformation in UE4.
...
#### Implementation
1. Enable the skeletalmeshComponent Render CustomDepth Pass under the rendering tag of details. This option allows the character to be rendered in the CustomDepth Pass, whether it is visible or not. This allows us to access the depth of the character at any time by reading the customDepth buffer.
...
4. In the material named Depth in the project file, the comparison between CustomDepth and SceneDepth can determine the position where the character standing on. Mark the place where the character stands on as black and the rest as red in texture space. Then, save the post-processing results into renderTarget2D named Snow_Scene_Captur_RTT1.
5. In the landscape rendering material, Snow_Scene_Captur_RTT1 can be used as LUT to find the places where character have stepped on in world space. Tessellation and world displacement then are used to achieve snow deformation.
6. In order to save the character's footprint, we also need to draw Snow_Scene_Captur_RTT1 into Snow_Scene_Captur_RTT2 (renderTarget2D) using a material named Snow_ADDRTT for each frame. In step 4, we need to blend Snow_Scene_Captur_RTT2 to genrate the final result for updating Snow_Scene_Captur_RTT1, because it consists of the information of character's footprint in last frame.   
...
Initially, the captureComponent2D I used was fixed to the scene. In our previous versions, the scene was small (2048 * 2048). CaptureComponent2D can easily cover the whole scene and capture the depth of the whole scene. But as the project progressed, our scene grew larger and larger (30,000 * 30,000). Using captureComponent2D capture on such a large scale is devastating on performance. The large size of renderTarget2D also have a significant impact on the performance of the game.   
#### Optimization and solution
1. Let captureComponent2D move with character and limit the capture scope.   
...
By default, captureComponent2D takes a snapshot of a scene with applying light, shadow, and reflection, but we only need depth in our program.  So, we can save performace cost by turning off unnecessary rendering options by adjusting the options under SceneCapture tag in detail setting of the component.
...
There is a BlueprintImplementableEvent declared in TPSCharacterQ.h. When characters using his gun to collect  snow, we make it interact  with the BP_PlayerCharaccter blueprint by calling this function. The position of Snow collection will be recorded into Snow_Scene_Captur_RTT2(mentioned above) by using Hit Snow Material in the blueprint to achieve Snow deformation.    
...
A noise texture is used in enemy's material to simulate the random distribution of snow, and the Fresnel and subsurface Shading model are to simulate the color of snow and ice. WorldPositionOffset output is used to produce icicle. In the enemy's material I blend the frozen color and the enemy's original color. Also, there is a variable exposed to control the amount of snow on the enemy. This allows us to achieve the effect that enemies are gradually frozen by our frost cannon.   
...
#### Issue
In the first version, I used tessellation and worldDisplacement to generate icicle. But when there were 20 enemies in the screen at the same time, the frame rate become very low. That is because the mesh of our enemy has 34000+ vertices the and 25000+ triangles, if we still use the tessellation, it would be a huge performance hit.
#### Solution
The tessellation was cancelled and icicle was generated by using world position offset instead of worldDisplacement in current version. The result of using world position offset was similar to that of tessellation no our high-ploy model. Tessellation is more suitable for adding more details to the low-ploy model rather than on high-ploy model.
...
In addition to the problems and solutions mentioned above, here are some tips for future students.
